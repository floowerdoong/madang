...
/// 설정을 **찾을** 자리들. 앞에서부터 본다.
///
/// ⚠️ **설정 폴더를 지정하면 거기만 본다.** 예전에는 지정한 폴더에 그 파일이 아직 없으면
/// 아래 자리들로 계속 내려가 **진짜 설정을 집어 왔다.** 그래서 확인용 판을 빈 폴더로 띄워도
/// 대표의 등록 폴더 14개가 그대로 떴다(2026-09-21 빈 집 확인에서 발견).
///
/// 판을 가르려고 주는 값인데 반대로 새면, 그 판으로 찍은 화면에 진짜 데이터가 섞인다.
/// 이사 편의(옛 자리에서 찾아오기)는 **지정이 없을 때만** 남긴다.
...
final List<String> kConfigRoots = configRoots(
  configDir: Platform.environment['CLAUDE_WATCHER_CONFIG_DIR'],
  cwd: Directory.current.path,
  buildTree: _buildTreeRoot(),
  besideApp: _besideApp(),
...
class OwnerNotices {
  ...
  String takeFor(String sessionId, String cwd, List<ProjectRow> projects) {
    ...
    return '[Madang] ${OwnerLabel.value}가 할 일 페이지에서 바꾼 것 — 이 태스크들의 상태는 이 값을 따른다:\n'
        '${lines.join('\n')}\n';
  }
}
...
class OfficeLayout {
  ...
  String noticeFor(String sessionId, String cwdPath, String folderName) {
    ...
    if (now.isEmpty) {
      ...
      return '[Madang] ${OwnerLabel.value}가 이 세션의 별명을 지웠다. 이제 폴더 이름 「$folderName」으로 부른다.\n';
    }
    if (before == now) return '';
    return '[Madang] 이 세션(폴더 「$folderName」)의 이름은 「$now」이다. '
        '${OwnerLabel.value}가 대화에서 「$now」라고 부르면 바로 이 세션을 말하는 것이다. 자기를 소개할 때도 이 이름을 쓴다.\n';
  }
}
...
/// 앱이 **쓰는 사람을 뭐라고 부를지.** 화면 글자와 세션에 보내는 말에 쓴다.
///
/// ⚠️ **저장되는 값은 [kOwnerAssignee]('owner')이고 이건 보이는 이름뿐이다.**
/// 둘을 섞으면 이름을 바꾸는 순간 옛 태스크의 담당이 사라진다.
///
/// 기본값이 「사용자」인 이유 — 만든 곳에서는 「대표」라고 부르지만 그건 그 회사의 말이다.
/// 받아 쓰는 사람에게 자기 회사 호칭을 강요하지 않는다. `owner.json`에 한 줄 적으면 바뀐다
/// (2026-09-21 범용성 정리).
class OwnerLabel {
  static String get _path => resolveConfigPath(
      kIsDevInstance ? 'owner.dev$kPort.json' : 'owner.json');

  static String? _cached;

  static String get value => _cached ??= _load();

  static String _load() {
    try {
      final f = File(_path);
      if (f.existsSync()) {
        final got = jsonDecode(f.readAsStringSync())['name'];
        if (got is String && got.trim().isNotEmpty) return got.trim();
      }
    } catch (e) {
      debugPrint('부를 이름 읽기 실패: $e');
    }
    return '사용자';
  }
  ...
  static bool save(String name) {
    ...
    try {
      File(_path).writeAsStringSync(jsonEncode({'name': v}));
      _cached = v;
      return true;
    } catch (e) {
      debugPrint('부를 이름 저장 실패: $e');
      return false;
    }
  }
}
...
class TodoStore {
  ...
  static String? describeOwnerChange(TodoItem? before, TodoItem? after) {
    ...
    final parts = <String>[
      ...
      if (before.assignee != after.assignee)
        '담당 → ${after.assignee == null ? '없음' : after.assignee == kOwnerAssignee ? OwnerLabel.value : after.assignee!.split('/').last}',
  ...
  static String? sendRefusal(TodoItem t, AgentStatus? targetStatus) {
    if (t.assignee == kOwnerAssignee) return '${OwnerLabel.value} 담당이라 세션에 보내지 않는다';
...
.setupsheet .ownerin{width:140px;padding:5px 10px;border:1px solid var(--line);border-radius:999px;background:var(--card);color:var(--fg);font:inherit;font-size:13px}
...
async function phoneSheet(){
  ...
  // 꺼져 있으면 주소 대신 켜는 버튼을 낸다 — 입구를 여는 것은 사람이 정한다(2026-09-21).
  if(!j.on){
    sh.innerHTML = '<div class="sh"><div class="title">폰으로 보기</div><button class="ic x" title="닫기 (Esc)" onclick="closeSheet()">✕</button></div>'
      + '<p class="phn">폰에서 할 일을 보려면 이 맥에 <b>입구를 하나 열어야</b> 한다. 지금은 닫혀 있다.<br>'
      + '같은 와이파이(또는 <b>Tailscale</b>)에 있는 기기에서 열쇠가 있어야 열린다. <b>평문 HTTP</b>이니 카페 와이파이에서는 켜지 않는다.</p>'
      + '<div class="sa"><button type="button" class="go primary" data-phone-on>폰 입구 열기</button>'
      + '<small>언제든 이 창에서 다시 닫는다. 공유기에 포트를 뚫는 것과는 다른 이야기고, 그건 하지 않는다</small></div>';
    const on = sh.querySelector('[data-phone-on]');
    on.onclick = async () => { if(await post('app/phone', {on: true}, on)) phoneSheet(); };
    openSheet('phone');
    return;
  }
  ...
  sh.innerHTML = '<div class="sh"><div class="title">폰으로 보기</div><button class="ic x" title="닫기 (Esc)" onclick="closeSheet()">✕</button></div>'
    ...
    + '<div class="sa foot"><button type="button" class="txtbtn" data-phone-off>폰 입구 닫기</button></div>';
  const off = sh.querySelector('[data-phone-off]');
  if(off) off.onclick = async () => { if(await post('app/phone', {on: false}, off)) phoneSheet(); };
...
async function setupSheet(auto){
  ...
  sh.innerHTML = '<div class="sh"><div class="title">처음 설정</div><button class="ic x" title="닫기 (Esc)" onclick="closeSheet()">✕</button></div>'
    ...
    + '<div class="agpick"><b>부를 이름</b><span class="faint">앱이 당신을 뭐라고 부를지 — 화면과 세션에 보내는 말에 쓴다</span>'
      + '<div><input id="ownername" class="ownerin" maxlength="20" value="' + esc(j.owner || '사용자') + '"><button type="button" class="go" data-owner>바꾸기</button></div></div>'
  ...
  const ob = sh.querySelector('[data-owner]');
  if(ob) ob.onclick = async () => {
    const v = (sh.querySelector('#ownername') || {}).value || '';
    if(await post('app/owner', {name: v}, ob)) setupSheet(false);
  };
  ...
  switch (path) {
    ...
    case '/todo/api/tasks/status':
      ...
      if (st == TaskStatus.done && data['ownerConfirmed'] != true) {
        return fail('완료는 ${OwnerLabel.value}가 「완료처리해줘」라고 했을 때만 — ownerConfirmed: true를 붙인다');
      }
    ...
    case '/todo/api/tasks/stop':
      ...
      if (!t.ticking) {
        return fail('시계가 이미 멈춰 있다(지금 ${t.status.label}) — 다른 태스크를 시작했거나, 세션이 끝났거나, ${OwnerLabel.value}가 화면에서 옮겼다. '
...
String todosToCsv(List<TodoItem> items, ProjectDb db) {
  ...
  for (final t in items) {
    ...
    final who = t.assignee == null
        ? ''
        : (t.assignee == kOwnerAssignee ? OwnerLabel.value : t.assignee!.split('/').last);
    buf.writeln([
...
let mtgSig = '';
const MTG_STATE = {'발언중': ['blue', '참석자가 발언을 쓰는 중'], '정리중': ['purple', '사회자가 라운드를 정리하는 중'], '의견대기': ['yellow', '${OwnerLabel.value} 의견을 기다리는 중 — 사회자 세션에 말한다'], '완료': ['green', '회의가 끝났다']};
...
function drawMtg(j){
  ...
  if(m.done && (m.draft || m.userNotes.length)){
    ...
    d.append(el('summary', '', '마지막 라운드 정리' + (m.userNotes.length ? ' · ${OwnerLabel.value} 의견 ' + m.userNotes.length + '건' : '')));
    if(m.draft){ const b = el('div', 'ch-text'); b.innerHTML = md(m.draft); d.append(b); }
    if(m.userNotes.length){ const ul = el('ul'); m.userNotes.forEach(x => ul.append(el('li', '', x))); d.append(el('b', '', '${OwnerLabel.value} 의견'), ul); }
    log.append(d);
  } else if(m.draft){
    ...
    c.append(el('b', '', m.state === '정리중' || spoke === m.speakers.length ? '사회자가 결론을 정리하는 중' : '발언을 기다리는 중 — ' + spoke + '/' + m.speakers.length),
      el('p', 'faint', '결론이 나오면 여기에 ${OwnerLabel.value} 의견 칸이 열린다.'));
    log.append(c);
  }
  if(m.userNotes.length && !m.done){
    const c = el('div', 'mtg-card'); c.append(el('b', '', '${OwnerLabel.value} 의견'));
...
String todoPageHtml(SessionStore store, Todos todos, ProjectDb db,
  ...
  String whoName(String? a) =>
      a == null ? '' : (a == kOwnerAssignee ? OwnerLabel.value : nameOf(a));
  String assigneeOptions(String? selected) => [
        '<option value=""${selected == null ? ' selected' : ''}>—</option>',
        '<option value="$kOwnerAssignee"${selected == kOwnerAssignee ? ' selected' : ''}>${OwnerLabel.value}</option>',
  ...
  for (final l in BallLane.values) {
    ...
    if (mine.isEmpty) {
      cards.write('<div class="empty">${switch (l) {
        BallLane.me => '${OwnerLabel.value} 차례가 비었다.',
  ...
  final panel = StringBuffer('<div class="prow head"><span></span><span>프로젝트</span>'
      '<span title="내 차례">${OwnerLabel.value}</span><span title="세션 차례 · 진행중">세션</span><span>백로그</span><span>이번 주</span></div>');
...
${listOf(myToday, scope == TodoScope.today ? '오늘 ${OwnerLabel.value}가 할 일은 확인필요뿐이다. 「이번 주」로 넓혀 본다.' : '이 범위에 ${OwnerLabel.value} 몫이 없다.', implied: TaskStatus.today, lane: 'today', laneLabel: laneShort(BallLane.me))}</section>
...
Future<void> startHookServer(SessionStore store, Todos todos,
  ...
  try {
    ...
    await for (final request in server) {
      ...
      // 남의 웹페이지가 보낸 것은 여기서 끊는다 — 훅이든 할 일이든 똑같이 막는다.
      if (_fromOtherSite(request)) {
        debugPrint('다른 사이트에서 온 요청을 막았다: $path '
            '← ${request.headers.value('origin') ?? request.headers.value('sec-fetch-site')}');
        request.response.statusCode = 403;
        request.response.write('{"ok":false,"error":"다른 사이트에서 온 요청은 받지 않는다"}');
...
/// 폰 입구를 열어 둘지. **기본은 끔이다.**
///
/// ⚠️ 예전에는 앱을 켜면 조건 없이 `0.0.0.0`을 열었다. 받은 사람은 자기 맥에
/// **망으로 열린 포트가 하나 생긴 줄 모른다.** 열쇠가 있어 아무나 못 열지만 평문 HTTP라,
/// 카페 와이파이에서 켜면 같은 망 사람에게 포트가 보인다(2026-09-21 빈 집 확인에서 드러났다).
///
/// 그래서 **쓰겠다고 한 사람에게만 연다.** ⚙ 「폰으로 보기」에서 켜면 그때 뜬다.
class PhoneAccess {
  static String get _path => resolveConfigPath(
      kIsDevInstance ? 'phone.dev$kPort.json' : 'phone.json');

  static bool? _cached;

  static bool get on => _cached ??= _load();

  static bool _load() {
    try {
      final f = File(_path);
      if (f.existsSync()) return jsonDecode(f.readAsStringSync())['on'] == true;
    } catch (e) {
      debugPrint('폰 입구 설정 읽기 실패: $e');
    }
    return false;
  }

  static void save(bool v) {
    _cached = v;
    try {
      File(_path).writeAsStringSync(jsonEncode({'on': v}));
    } catch (e) {
      debugPrint('폰 입구 설정 저장 실패: $e');
    }
  }
}

/// 지금 열려 있는 폰 입구. 꺼져 있으면 null.
HttpServer? kPhoneServer;

/// 폰 입구를 여는 길. `main`이 한 번 채운다 — 설정에서 켤 때 부른다.
Future<void> Function()? kOpenPhone;
...
Future<void> startTodoServer(
  ...
  if (kPhoneServer != null) return; // 이미 열려 있다
  try {
    final server = kPhoneServer = await HttpServer.bind(InternetAddress.anyIPv4, kTodoPort);
    debugPrint('폰에서 여는 할 일: http://<이 맥의 주소>:$kTodoPort'
        '/todo?k=${TodoKey.value}');
    // ⚠️ **받는 고리를 기다리지 않는다.** 여기서 `await for`를 하면 이 함수가 영영 안 끝나서,
    // 설정에서 「켜기」를 부른 쪽이 답을 못 받고 멎는다(2026-09-21에 실제로 그랬다).
    // 문이 열린 것까지만 알리고, 받는 일은 뒤에서 돈다.
    unawaited(() async {
      try {
        await for (final request in server) {
...
/// 남의 웹페이지가 보낸 요청인가.
///
/// ⚠️ **`127.0.0.1`에 묶었다고 안전한 것이 아니다.** 대표가 아무 웹사이트나 열어 두면
/// 그 페이지가 `http://127.0.0.1:9876` 으로 POST를 보낼 수 있다. 답은 못 읽지만(CORS)
/// **쓰는 것은 된다** — 실제로 찔러 보니 할 일이 들어갔고 `/todo/chat/send` 까지 통했다
/// (2026-09-21 실측). 그러면 남의 페이지가 대표 세션에 말을 집어넣는다.
///
/// 그래서 브라우저가 붙여 주는 표식을 본다. 브라우저는 `Origin`·`Sec-Fetch-Site` 를
/// **자바스크립트가 못 고치게** 붙인다. 훅의 curl·앱 자신은 그런 헤더가 없어서 그냥 지난다.
/// 대시보드 화면은 같은 자리(`127.0.0.1:9876/todo`)에서 열리므로 `same-origin` 으로 온다.
bool fromOtherSite(String? site, String? origin, int port) {
  if (site != null) return site != 'same-origin' && site != 'none';
  if (origin == null) return false; // 브라우저가 아니다 (훅·스크립트)
  return origin != 'http://127.0.0.1:$port' && origin != 'http://localhost:$port';
}

bool _fromOtherSite(HttpRequest request) => fromOtherSite(
    request.headers.value('sec-fetch-site'),
  ...
  try {
    ...
    if (request.method == 'GET' && request.uri.path == '/todo/app/phone') {
      ...
      if (needsKey) {
        ...
        final addrs = PhoneAccess.on ? await phoneAddresses() : const <({String label, String address})>[];
    ...
    if (request.method == 'POST') {
      ...
      switch (request.uri.path) {
        ...
        // 앱이 쓰는 사람을 뭐라고 부를지. 받아 쓰는 사람이 자기 호칭을 정한다(2026-09-21).
        case '/todo/app/owner':
          if (needsKey) {
            error = '폰에서는 바꿀 수 없다';
          } else if (!OwnerLabel.save(data['name'] as String? ?? '')) {
            error = '한 글자에서 스무 글자 사이로 적는다';
          }
          break;
        // 폰 입구를 켜고 끈다. **기본은 꺼져 있다** — 여는 것은 사람이 정한다(2026-09-21).
        case '/todo/app/phone':
          if (needsKey) {
            error = '폰에서는 바꿀 수 없다';
          } else {
            final want = data['on'] == true;
            PhoneAccess.save(want);
            if (want) {
              await kOpenPhone?.call();
              if (kPhoneServer == null) error = '폰 입구를 못 열었다 — 포트가 쓰이고 있을 수 있다';
            } else {
              await kPhoneServer?.close(force: true);
              kPhoneServer = null;
            }
          }
          break;
        ...
        case '/todo/chat/rename':
          ...
          final say = now == null
              ? '[마당] ${OwnerLabel.value}가 이 세션의 별명을 지웠다. 이제 폴더 이름 「${s.folderName}」으로 부른다.'
              : '[마당] ${OwnerLabel.value}가 이 세션의 이름을 「$now」로 정했다. 앞으로 ${OwnerLabel.value}가 「$now」라고 부르면 이 세션을 말하는 것이다. 지금 하던 일은 그대로 이어서 한다.';
...
class Tmux {
  ...
  /// 통째로 넘기면 인자 한도에 걸린다.
  /// 화면에서 **입력창 한 칸**만 떠낸다. 없으면 null.
  ///
  /// 클로드 코드는 입력창을 가로줄(`───`) 두 개 사이에 그린다. 그 사이만 보는 이유는
  /// 아래 여덟 줄을 통째로 보면 **줄 서 있는 말**(작업 중에 보내면 입력창 위에 `❯ …`로
  /// 쌓인다)까지 걸리기 때문이다. 그러면 이미 들어간 말을 「아직 안 들어갔다」로 읽고
  /// Enter를 더 친다 — 그 Enter가 하필 선택창에 떨어지면 사람 대신 골라 버린다
  /// (2026-09-21 데모 세션에서 실측: 같은 말이 화면 위치에 따라 걸리기도 하고 안 걸리기도 했다).
  static String? inputBox(String pane) {
    final lines = pane.split('\n');
    final rules = <int>[];
    for (var i = 0; i < lines.length; i++) {
      final t = lines[i].trim();
      if (t.length >= 20 && RegExp(r'^[─━-]+$').hasMatch(t)) rules.add(i);
    }
    if (rules.length < 2) return null;
    final close = rules.last;
    final open = rules[rules.length - 2];
    if (close - open < 1) return null;
    return lines.sublist(open + 1, close).join('\n');
  }

  /// 입력창이 아직 그 말을 붙잡고 있나. 입력창을 못 찾으면 예전처럼 아래 여덟 줄로 본다
  /// (코덱스처럼 화면 모양이 다른 경우).
  static bool inputHolds(String pane, String probe) {
    if (probe.isEmpty) return false;
    final box = inputBox(pane);
    if (box != null) return box.contains(probe);
  ...
  static Future<bool> sendLine(String name, String line) async {
    ...
    try {
      ...
      for (var tries = 0; tries < 2 && probe.isNotEmpty; tries++) {
        ...
        if (!inputHolds(pane, probe)) return true; // 입력창에서 사라졌다 = 들어갔다
        debugPrint('보낸 말이 입력창에 남아 있다 — Enter를 다시 친다 ($name)');
        await Process.run(bin, ['send-keys', '-t', name, 'Enter']);
      }
      await Future<void>.delayed(const Duration(milliseconds: 450));
      final last = await capturePane(name);
      final stuck = last != null && inputHolds(last, probe);
...
Future<void> _startEverything(bool dashboardMode) async {
  ...
  // 폰 입구는 **쓰겠다고 한 사람에게만** 연다(기본 끔). ⚙ 「폰으로 보기」에서 켜면 그때 뜬다.
  kOpenPhone = () => startTodoServer(store, todos, projects, projectDb, workLog, sessionLog);
  if (PhoneAccess.on) unawaited(kOpenPhone!());
