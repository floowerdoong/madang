...
Future<String?> sendToSession(
  ...
  if (!await Tmux.sendLine(name, text)) return '보내지 못했다 ($name)';
...
class SessionStore extends ChangeNotifier {
  ...
  void _notifyMac(String name, String path) {
    ...
    unawaited(() async {
      try {
        await kNotify.invokeMethod<bool>('post', {'title': name, 'body': '작업이 끝났습니다 — 눌러서 대화 열기', 'path': path});
      } catch (_) {}
    }());
  }
...
/* 건물 위로 남는 자리는 **하늘**이다 — 시간대에 따라 색이 바뀐다. 비어 있는 흰 바닥으로 두면
   건물이 아니라 「목록이 아래로 밀린 것」처럼 보인다(리뷰 9/18 뒤 손질). */
.tree{display:flex;flex-direction:column;gap:0;padding:8px 6px 10px;justify-content:flex-end;
  background:var(--of-sky,none);background-repeat:no-repeat;background-position:top;background-size:100% 70%}
.tree.tod-morning{--of-sky:linear-gradient(180deg,rgba(255,214,150,.30),rgba(255,214,150,0) 85%)}
.tree.tod-afternoon{--of-sky:linear-gradient(180deg,rgba(150,200,240,.28),rgba(150,200,240,0) 85%)}
.tree.tod-evening{--of-sky:linear-gradient(180deg,rgba(40,48,92,.42),rgba(40,48,92,0) 85%)}
.bldg{border-radius:10px;overflow:hidden;border:1px solid var(--line2);margin-top:auto}
.bldg-roof{height:9px;border-radius:10px 10px 0 0;background:linear-gradient(180deg,var(--of-floorline),rgba(0,0,0,0));opacity:.55}
...
.tree.title-band .team .lift{top:auto;bottom:calc(var(--of-band,26px) - 15px);right:8px}
...
.team .lift{position:absolute;right:60px;top:2px;display:flex;gap:2px;opacity:0;transition:opacity .12s}
.team:hover .lift,.team .lift:focus-within{opacity:1}
.team .lift button{width:18px;height:16px;border:1px solid var(--line2);border-radius:5px;background:var(--panel);color:var(--dim);font-size:9px;line-height:1;cursor:pointer}
.team .lift button:hover:not(:disabled){color:var(--fg);background:var(--hover)}
...
.setupsheet .su.ok .mark{background:var(--accent)}
...
async function setupSheet(auto){
  ...
  const rows = j.items.map(it => {
    ...
    let act = '';
    if(!it.ok && it.key === 'hooks' && !j.broken) act = '<button type="button" class="go primary" data-do="app/setup-hooks">훅 연결하기</button><small>바뀌는 것: ~/.claude/settings.json 에 위의 빠진 이벤트마다 「세션 상태를 127.0.0.1로 알리는 한 줄」을 덧붙인다 · 먼저 백업(settings.json.bak-시각)을 남기고 남의 훅은 건드리지 않는다 · 언제든 「훅 빼기」로 되돌린다</small>';
    ...
    if(it.key === 'codexHooks'){
      act = (it.ok ? '' : '<button type="button" class="go primary" data-do="app/setup-codex-hooks">Codex 훅 연결하기</button>')
        + '<small>' + (it.ok ? '' : '바뀌는 것: ~/.codex/hooks.json 에 이벤트마다 「127.0.0.1로 알리는 한 줄」을 덧붙인다(백업을 남기고 남의 훅은 그대로) · ')
        + '<b>코덱스는 새 훅을 직접 신뢰해야 돈다</b> — 코덱스 안에서 <code>/hooks</code> 를 열고 <b>t</b>(전부 신뢰)를 누른다</small>'
    ...
    return '<div class="su ' + (it.ok ? 'ok' : it.optional ? 'opt' : 'no') + '"><span class="mark">' + (it.ok ? '✓' : it.optional ? '·' : '!') + '</span><div class="st"><b>' + esc(name) + '</b> <span class="faint">' + esc(it.detail) + '</span>'
      + '<div class="faint">' + esc(why) + '</div>' + (act ? '<div class="sa">' + act + '</div>' : '') + '</div></div>';
  }).join('');
  ...
  sh.innerHTML = '<div class="sh"><div class="title">처음 설정</div><button class="ic x" title="닫기 (Esc)" onclick="closeSheet()">✕</button></div>'
    ...
    + '<p class="phn">' + (j.done ? '다 됐다. 등록한 폴더에서 <b>' + agName + '</b> 를 켜면 사무실에 캐릭터가 뜬다. 이미 켜 둔 세션은 다시 켜야 훅이 붙는다.' : '모두 ✓ 가 되면 쓸 수 있다.') + '</p>'
...
function drawOfficeNow(tree){
  ...
  tree.replaceChildren(el('div', 'bldg-roof'), bldg);
...
function drawHeadNow(){
  ...
  who.append(face(s), el('b', 'nm', s.name), dot);
  ...
  more.onclick = ev => {
    ...
    openMenu(more, m => {
      ...
      rn.onclick = () => { m.remove(); renameMenu(more, s); };
...
function mtgConfirm(busy, notReady, yes){
  ...
  if(busy.length || notReady.length){
    card.append(el('h4', '', busy.length ? '현재 참석할 세션이 작업 중입니다' : '아직 출근하는 중인 세션이 있습니다'));
