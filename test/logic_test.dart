...
''';
  ...
  group('보낸 말이 들어갔나 — 입력창만 본다', () {
    // 2026-09-21 데모 세션에서 떠온 모양. 작업 중에 보낸 말은 입력창 위에 「❯ …」로 줄을 선다.
    const queued = '❯ bash로 sleep 20 을 실행해줘\n'
        '\n'
        '  Ran 1 shell command\n'
        '\n'
        '❯ 검증메시지DDD444 안전망이 이 말을 어떻게 보나\n'
        '\n'
        '✳ Channeling… (20s)\n'
        '────────────────────────────────────────────────\n'
        '❯ \n'
    ...
    test('줄 선 말은 들어간 것으로 본다', () {
      expect(Tmux.inputHolds(queued, '검증메시지DDD444'), isFalse);
    });

    test('입력창에 남아 있으면 안 들어간 것이다', () {
      expect(Tmux.inputHolds(stuck, '검증메시지DDD444'), isTrue);
    });

    test('입력창만 떠낸다', () {
      expect(Tmux.inputBox(stuck)?.trim(), '❯ 검증메시지DDD444 안전망이 이 말을 어떻게 보나');
      expect(Tmux.inputBox(queued)?.trim(), '❯');
    });

    test('가로줄이 없으면 예전처럼 아래 여덟 줄로 본다', () {
      expect(Tmux.inputHolds('한 줄\n검증메시지DDD444', '검증메시지DDD444'), isTrue);
      expect(Tmux.inputHolds('한 줄\n다른 말', '검증메시지DDD444'), isFalse);
    });
  });

  group('남의 웹페이지가 보낸 요청은 막는다 (2026-09-21 실측)', () {
    test('대시보드 자신이 보낸 것은 지난다', () {
      expect(fromOtherSite('same-origin', 'http://127.0.0.1:9876', 9876), isFalse);
      expect(fromOtherSite(null, 'http://localhost:9876', 9876), isFalse);
    });

    test('주소창에 쳐서 연 것(none)도 지난다', () {
      expect(fromOtherSite('none', null, 9876), isFalse);
    });

    test('훅의 curl처럼 헤더가 없으면 지난다', () {
      expect(fromOtherSite(null, null, 9876), isFalse);
    });

    test('남의 사이트에서 온 것은 막는다', () {
      expect(fromOtherSite('cross-site', 'https://evil.example', 9876), isTrue);
      expect(fromOtherSite(null, 'https://evil.example', 9876), isTrue);
    });

    test('같은 사이트라도 다른 포트면 막는다 — 확인용 판을 흉내 낼 수 있다', () {
      expect(fromOtherSite(null, 'http://127.0.0.1:9877', 9876), isTrue);
    });

    test('sec-fetch-site가 오면 그것을 먼저 믿는다', () {
      expect(fromOtherSite('same-site', 'http://127.0.0.1:9876', 9876), isTrue);
    });
  });

  group('설정을 찾을 자리 — 지정하면 거기만 본다 (2026-09-21)', () {
    test('설정 폴더를 주면 그 하나만 본다', () {
      expect(
        configRoots(
          configDir: '/tmp/판가르기',
          cwd: '/일터',
    ...
    test('안 주면 예전대로 여러 자리를 훑는다 — 옛 자리에서 이사해 오려고', () {
      expect(
        configRoots(
          cwd: '/일터',
          buildTree: '/빌드',
    ...
    test('빈 글자는 안 준 것으로 본다', () {
      expect(configRoots(configDir: '', cwd: '/일터', supportDir: '/지원'),
          ['/일터', '/지원']);
    });

    test('같은 자리는 한 번만 넣는다', () {
      expect(configRoots(cwd: '/같음', besideApp: '/같음', supportDir: '/같음'),
          ['/같음']);
    });
  });
}
