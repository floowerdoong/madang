...
세션을 두세 개 띄우기 시작하면 터미널 탭이 금세 일곱 개가 됩니다. 그때부터는 어느 세션이 답을 끝냈는지,
어느 세션이 승인을 기다리며 멈춰 있는지 탭을 하나씩 눌러 봐야 알 수 있습니다.

Madang은 그 세션들을 한 화면의 사무실로 모읍니다. 층마다 팀이 앉고, 일하는 세션은 움직이고,
승인을 기다리는 세션에는 표가 붙습니다. 할 일을 적어 「시키기」를 누르면 그 폴더의 세션이 받아 갑니다.

`macOS 12+` · `Apple Silicon` · 애플 공증 완료 · MIT

어떻게 쓰는지 보기 → **[muwidarani.com/how-we-work](https://muwidarani.com/how-we-work/)**

![세션이 할 일을 받아 일하는 화면](docs/shots/00_demo.gif)

## 무엇을 하나요

**적고 시킵니다.** 할 일을 리스트·보드로 적어 두고 「시키기」를 누르면 그 폴더의 세션이 받아 갑니다.
세션도 스스로 할 일을 만들고 시작·멈춤 시간을 남기므로, 무엇을 얼마나 했는지가 저절로 쌓입니다.

![「시키기」를 누르면 그 폴더의 세션이 받아 간다](docs/shots/10_send.gif)

![끝난 일은 「확인필요」로 돌아온다 — 눌러서 완료한다](docs/shots/11_done.gif)

고칠 것이 있으면 카드에서 바로 수정 요청을 적어 보냅니다. 세션이 그 말을 받아 이어서 고칩니다.

![고칠 것이 있으면 카드에서 바로 「수정요청」을 보낸다](docs/shots/13_revise.gif)

**봅니다.** 폴더마다 도는 세션이 사무실의 캐릭터로 앉습니다. 생각 중인지 도구를 쓰는 중인지 승인을
기다리는지가 한눈에 보이고, 아무도 켜지 않은 층은 불이 꺼져 있습니다.

![Madang 대시보드 — 왼쪽은 할 일, 오른쪽은 세션이 앉은 사무실](docs/shots/01_dashboard.png)

일이 여러 프로젝트에 걸쳐 있으면 프로젝트 이름을 눌러 그것만 봅니다.

![프로젝트를 누르면 그 프로젝트 일만 보인다](docs/shots/12_projects.gif)

**주고받습니다.** 대화 칸에서 세션이 한 말을 읽고, 말을 보내고, 터미널에 뜬 승인창·선택창을 버튼으로
고릅니다. 터미널로 건너갈 일이 거의 없습니다.

여러 세션을 한 주제로 모아 라운드 토론을 시키는 회의실, 남은 사용 한도와 세션별 토큰, 폰에서 보기도
들어 있습니다.
...
한 사람이 세션 아홉 개를 굴리는 날이 있었습니다. 프로젝트마다 하나씩 띄워 두니 터미널 탭이 아홉 개가 되고,
어느 탭이 승인을 기다리며 멈춰 있는지 찾느라 한 바퀴를 돌아야 했습니다. 답이 끝난 줄 모르고 십 분을 흘려보내는
일도 잦았습니다.

그래서 「사람이 탭을 도는」 대신 「세션이 자기 자리에 앉아 상태를 내보이는」 쪽으로 뒤집었습니다.
화면 한켠에 사무실을 두고, 누가 일하는 중이고 누가 기다리는지를 눈으로 보게 만든 것이 Madang입니다.

1인 사업자가 실제로 일하면서 쓰는 도구라 기록도 같이 남습니다. 무엇을 시켰고 얼마나 걸렸는지가 할 일에 쌓이고,
세션 스스로도 시작·멈춤을 적습니다.

## 무엇으로 만들었나

Flutter(macOS)로 만든 앱이고, 대시보드 화면은 앱이 직접 띄우는 로컬 서버가 그리는 HTML을 WKWebView에 얹은 것입니다.
세션은 앱에 동봉한 tmux 위에서 Claude Code·Codex CLI로 돌고, 상태는 두 도구의 훅이 `127.0.0.1:9876`으로 보내 줍니다.
저장은 이 맥 안의 JSON 파일뿐이라 서버도 계정도 없습니다.

이 저장소의 코드는 Claude Code가 썼습니다. 무엇을 만들고 무엇을 고칠지는 사람이 정했고,
쓰면서 걸린 것을 그때그때 고치는 식으로 자랐습니다.
설계에서 지킨 것과 겪은 함정은 [`CLAUDE.md`](CLAUDE.md)에 그대로 적혀 있습니다.

## 설치

### 1. 준비물

아래 둘 중 하나 이상이 필요합니다. 둘 다 써도 됩니다.
...
![처음 설정 창 — 준비물과 훅 연결을 한 줄씩 점검한다](docs/shots/02_setup.png)
...
- **폰으로 보기** — ⚙ → 폰으로 보기. **입구는 기본으로 닫혀 있습니다.** 「폰 입구 열기」를 누르면 그때 열리고 QR이 나옵니다. 같은 와이파이, 또는 맥과 폰 모두 [Tailscale](https://tailscale.com)에 붙어 있어야 합니다. 주소에 열쇠가 들어 있으니 다른 사람에게 보내지 마세요. 같은 창에서 다시 닫습니다
...
- **회의실** — 사무실 맨 아래 「회의실」 → 회의 시작. 모을 세션을 고르고(퇴근한 세션은 누르면 출근시킵니다) 주제를 적으면, 가장 바깥 폴더의 세션이 진행자가 되어 라운드를 돌립니다. 진행자도 의견을 내므로 참석자 한 명과 1:1로도 됩니다. 라운드가 끝나면 결론 밑 의견 칸에 적어 「다음 회의 이어가기」 또는 「회의 끝내기」를 누릅니다. 끝난 회의의 `결론.md`는 대화 칸·사무실 얼굴로 끌어다 세션에 넘길 수 있습니다
  ...
  - 참석 세션의 승인 방식이 진행자와 다르면, 그 세션이 회의 메시지를 곧바로 받지 않고 사용자 승인을 기다립니다(승인하지 않으면 만료됩니다). Madang이 띄우는 세션은 모두 같은 표식([승인창에 대하여](#승인창에-대하여))을 따르므로 그대로 두면 맞습니다. 다만 승인 생략을 켜거나 끈 뒤에 띄운 세션부터 바뀌므로, 그 전에 켜 둔 세션은 한 번 다시 켜세요
...
기본은 도구 승인을 하나하나 묻는 쪽입니다. Madang이 띄우는 세션에서 승인을 생략하려면 표식 파일을 직접 만들어야 하고, 이미 떠 있는 세션에는 적용되지 않습니다. **내 코드만 여는 폴더에서만** 켜세요.
...
- 훅을 받는 포트(`127.0.0.1:9876`)는 이 맥 안에서만 열립니다. 다른 사이트에서 온 요청은 막지만(브라우저가 붙이는 표식을 봅니다), **같은 맥에서 도는 다른 프로그램이 신호를 흉내 내는 것은 아직 막지 못합니다**
- 폰으로 보기(`0.0.0.0:9878`)는 **기본으로 닫혀 있고**, 켠 동안에만 열립니다. 열쇠가 있어야 열리지만 평문 HTTP입니다. **공유기 포트를 열어 인터넷에 내놓지 마세요.** 카페 와이파이에서도 쓰지 않습니다
...
소스에서 빌드해 `~/Applications/Madang`(앱 옆에 `art/`가 있는 모양)으로 설치하면 설정·데이터는 `~/Library/Application Support/madang`이 아니라 그 폴더 안에 저장됩니다.

tmux가 앱에 들어가는 것은 릴리즈 DMG뿐입니다. 소스에서 빌드해 쓸 때는 `brew install tmux`로 깐 시스템 tmux를 씁니다.
