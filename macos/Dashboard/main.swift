...
class Dash: NSObject, NSApplicationDelegate, WKNavigationDelegate, WKUIDelegate, WKScriptMessageHandler, NSWindowDelegate {
  ...
  /// 창을 앞으로 꺼낸다.
  ///
  /// ⚠️ **최소화된 창은 `makeKeyAndOrderFront`로 안 나온다.** 노란 버튼으로 내려 둔 뒤 독 아이콘을 누르면
  /// 아무 일도 안 일어났고, 앱을 껐다 켜야 나왔다(대표 제보 2026-09-20).
  /// ⚠️ 저장해 둔 자리가 지금 화면 밖이면(모니터를 뺐다 꽂았다) 꺼내도 안 보인다 — 그때는 가운데로 데려온다.
  func showWindow() {
    ...
    if !onScreen, let screen = NSScreen.main {
      ...
      window.setFrameOrigin(NSPoint(x: v.midX - size.width / 2, y: v.midY - size.height / 2))
      log("창이 화면 밖이라 가운데로 데려왔다")
    }
    window.makeKeyAndOrderFront(nil)
    NSApp.activate(ignoringOtherApps: true)
  }
  ...
  /// 앞으로 불려 나올 때도 창을 꺼낸다 — 본 프로세스(Flutter)가 독 클릭을 받아 이 앱을 `activate`만
  /// 시키는 길이 있는데, activate는 **최소화된 창을 되돌리지 않는다**(대표 제보 9/20).
